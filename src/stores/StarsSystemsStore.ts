...
import { IStarSystemsConfig, IStarConfig, IPlanetoid } from "../types/StarsStoreTypes";
...
const state = reactive<IStarSystemsConfig>({
  StarSystemsConfig: <IStarConfig>{
    Solar: <Array<IPlanetoid>>[
      {
        nameId: 'Sun',
        type: 'star',
...
export default function useStarSystemsStore() {
  const getStarConfig = ((name: string) => {
    return state.StarSystemsConfig[name as keyof IStarSystemsConfig]
  })
